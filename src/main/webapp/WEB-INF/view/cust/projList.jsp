...
<head>
  ...
  <style type="text/css">
    ...
    */
    ...
    #projInsert {padding:5px 20px; border:0; color:#fff; background:#000; margin-top:5px;}
    #modal {display:none;
      width: 500px;
      height: 675px;
      position: absolute;
...
<div class="page-container">
  ...
  <div class="main-content">
    ...
    <div id = "wrapper">
      <button id="projInsert">프로젝트 등록</button>
      <table border="1" >
        <tr>
          <th>순번</th>
          <th>프로젝트명</th>
        ...
        <c:forEach items="${projList}" var="projList" varStatus="index">
          <tr>
            <td>${index.count}</td>
            <td>${projList.prjNm}</td>
            <td>${projList.prjAdr}</td>
            <td></td>
            <td></td>
            <td></td>
            <td>등급</td>
            <td>${projList.reqSkill}</td>
            <td>요청/관리</td>
          </tr>
        </c:forEach>
      </table>
    ...
    <div id="modal">
      ...
      <form id="projInsertData">
        <div>
          <h3>프로젝트 명</h3>
          <input type="text" id="prj_nm" name="prj_nm">
        </div>
        <div>
          <h3>필요기술</h3>
          <input type="text" id="req_skill" name="req_skill">
        </div>
        <div>
          <h3>등급 [단위 : 명]</h3>
          <input type="checkbox" id="cb_1" name="cb" ><span>초급</span><input type="text" id="lv_1" name="lv_1" class="people">
          <input type="checkbox" id="cb_2" name="cb" ><span>중급</span><input type="text" id="lv_2" name="lv_2" class="people">
          <input type="checkbox" id="cb_3" name="cb" ><span>고급</span><input type="text" id="lv_3" name="lv_3" class="people">
        </div>
        <div>
          <h3>프로젝트 근무지 주소</h3>
          <input type="text" name="prj_adr">
        </div>
        <div>
          <h3>프로젝트 시작</h3>
          <input type="text" id="str_yy" class="yydd"><span>년</span><input type="text" id="str_dd" class="yydd"><span>월</span>
        </div>
        <div>
          <h3>프로젝트 종료</h3>
          <input type="text" id="end_yy" class="yydd"><span>년</span><input type="text" id="end_dd" class="yydd"><span>월</span>
        </div>
        <div>
          <h3>요구사항</h3>
          <textarea cols="30" rows="10" name="memo"></textarea>
        </div>
      </form>
      <button id="projSave">저장</button>
...
<script>
  $(document).ready(function (){
    ...
    $("#projInsert").on("click", function(e){
      $("#modal").show();

    });
    $("#projSave").on("click", function(e){
      //checkbox 여부는 퍼블 나오면 만들기
      var str_yy = $("#str_yy").val();
      var str_dd = $("#str_dd").val();
      var end_yy = $("#end_yy").val();
      var end_dd = $("#end_dd").val();
      var str_res = str_yy.concat("-",str_dd);
      var end_res = end_yy.concat("-",end_dd);


      var totalStr = document.createElement("input");
      totalStr.setAttribute("type","date");
      totalStr.setAttribute("name","str_dt");
      totalStr.setAttribute("value",str_res);
      var totalEnd = document.createElement("input");
      totalEnd.setAttribute("type","date");
      totalEnd.setAttribute("name","end_dt");
      totalEnd.setAttribute("value",end_res);

      var formDada = $("#projInsertData");
      formDada.append(totalStr);
      formDada.append(totalEnd);
      formDada.attr("action","/cust/test2/insertData");
      formDada.attr("method","post");
      formDada.submit();
